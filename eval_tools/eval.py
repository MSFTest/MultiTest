...
from eval_tools.rotate_iou import rotate_iou_gpu_eval
...
@numba.jit(nopython=True)
def my_compute_statistics_jit(overlaps,
                              gt_datas,
                              dt_datas,
                              ignored_gt,
                              ignored_det,
    ...
    dt_scores2 = dt_scores.copy()
    dt_scores2 = dt_scores2 / prob_max
    ...
    delta_idx = 0
    my_gt_error_cnt, my_dt_error_cnt, my_loc_error_cnt = 0, 0, 0
    gt_index_list = []
    dt_index_list = []
    loc_index_list = []
    for i in range(gt_size):
        ...
        if flag_gt and ignored_gt[i] == 0:
            my_gt_error_cnt += 1
            gt_index_list.append(i)
    ...
    for j in range(det_size):
        dt_score = dt_scores[j]
        dt_score = min_max_prob(dt_score, prob_max)
        if dt_score < dt_prob_min:
            continue
        ...
        for i in range(gt_size):
            overlap = overlaps[j, i]
            if overlap > loc_error_iou_th:
                flag_loc = False
                flag_dt = False
                break
            elif dt_error_iou_th < overlap <= loc_error_iou_th:
                flag_loc = True
            elif overlap <= dt_error_iou_th:
                flag_dt = True
        if flag_loc:
            my_loc_error_cnt += 1
            loc_index_list.append(j)
        elif flag_dt:
            my_dt_error_cnt += 1
            dt_index_list.append(j)
    ...
    if compute_fp:
        ...
        if compute_aos:
            ...
            if tp > 0 or fp > 0:
                ...
                similarity = -1
    return tp, fp, fn, similarity, thresholds[:thresh_idx], (my_gt_error_cnt, my_dt_error_cnt, my_loc_error_cnt)
...
@numba.jit(nopython=True)
def my_fused_compute_statistics(overlaps,
                                pr,
                                gt_nums,
                                dt_nums,
                                dc_nums,
    ...
    for i in range(gt_nums.shape[0]):
        for t, thresh in enumerate(thresholds):
            ...
            ignored_det = ignored_dets[dt_num:dt_num + dt_nums[i]]
            dontcare = dontcares[dc_num:dc_num + dc_nums[i]]
            tp, fp, fn, similarity, _, (my_gt_error_cnt, my_dt_error_cnt, my_loc_error_cnt) = my_compute_statistics_jit(
                overlap,
                gt_data,
                dt_data,
                ignored_gt,
            ...
            pr[t, 1] += fp
            pr[t, 2] += fn
            gt_error_cnt += my_gt_error_cnt
            dt_error_cnt += my_dt_error_cnt
            loc_error_cnt += my_loc_error_cnt
            if similarity != -1:
                pr[t, 3] += similarity
        gt_num += gt_nums[i]
        dt_num += dt_nums[i]
        dc_num += dc_nums[i]
    return gt_error_cnt, dt_error_cnt, loc_error_cnt
...
def eval_class(gt_annos,
    ...
    total_gt_error = defaultdict(int)
    total_dt_error = defaultdict(int)
    total_loc_error = defaultdict(int)
        ...
        for l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for i in range(len(gt_annos)):
                    ...
                    tp, fp, fn, similarity, thresholds, (gt_error, dt_error, loc_error) = rets

                    total_fn[f"{current_class}_{difficulty}_{min_overlap}"] += fn
                    total_fp[f"{current_class}_{difficulty}_{min_overlap}"] += fp
                    total_gt_error[f"{current_class}_{difficulty}_{min_overlap}"] += gt_error
                    total_dt_error[f"{current_class}_{difficulty}_{min_overlap}"] += dt_error
                    total_loc_error[f"{current_class}_{difficulty}_{min_overlap}"] += loc_error
    ...
    ret_dict = {
        ...
        "total_fn": total_fn,
        "total_fp": total_fp,
        "total_gt_error": total_gt_error,
        "total_dt_error": total_dt_error,
        "total_loc_error": total_loc_error,
    }
    return ret_dict
...
def get_official_eval_result(gt_annos,
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            mAPbbox_ori = get_mAP_v2(metrics["bbox"]["precision"][j, :, i])
            mAPbbox = ", ".join(f"{v:.2f}" for v in mAPbbox_ori)
            mAPbev = get_mAP_v2(metrics["bev"]["precision"][j, :, i])
            mAPbev = ", ".join(f"{v:.2f}" for v in mAPbev)
            mAP3d_ori = get_mAP_v2(metrics["3d"]["precision"][j, :, i])
            mAP3d = ", ".join(f"{v:.2f}" for v in mAP3d_ori)
            result += print_str(
            ...
            if True:
                ...
                fp = metrics["3d"]["total_fp"]["0_1_0.5"]
                fn = metrics["3d"]["total_fn"]["0_1_0.5"]
                result += print_str(f"3d   fp   {fp}     fn {fn}")

                total_gt_error = metrics["3d"]["total_gt_error"]["0_1_0.5"]
                total_dt_error = metrics["3d"]["total_dt_error"]["0_1_0.5"]
                total_loc_error = metrics["3d"]["total_loc_error"]["0_1_0.5"]
                result += print_str(
                    f"3d   gt_error: {total_gt_error}   dt_error: {total_dt_error}  loc_error: {total_loc_error}")
                fp = metrics["bbox"]["total_fp"]["0_1_0.5"]
                fn = metrics["bbox"]["total_fn"]["0_1_0.5"]
                result += print_str(f"bbox   fp   {fp}     fn {fn}")
                total_gt_error = metrics["bbox"]["total_gt_error"]["0_1_0.5"]
                total_dt_error = metrics["bbox"]["total_dt_error"]["0_1_0.5"]
                total_loc_error = metrics["bbox"]["total_loc_error"]["0_1_0.5"]
                result += print_str(
                    f"bbox   gt_error: {total_gt_error}   dt_error: {total_dt_error}  loc_error: {total_loc_error}")
                if i == 1:
                    res_dict["mAP3d"] = mAP3d_ori[1]
                    res_dict["3d_gt"] = metrics["3d"]["total_gt_error"]["0_1_0.5"]
                    res_dict["3d_dt"] = metrics["3d"]["total_dt_error"]["0_1_0.5"]
                    res_dict["3d_loc"] = metrics["3d"]["total_loc_error"]["0_1_0.5"]
                    res_dict["bbox_AP"] = mAPbbox_ori[1]
                    res_dict["bbox_gt"] = metrics["bbox"]["total_gt_error"]["0_1_0.5"]
                    res_dict["bbox_dt"] = metrics["bbox"]["total_dt_error"]["0_1_0.5"]
                    res_dict["bbox_loc"] = metrics["bbox"]["total_loc_error"]["0_1_0.5"]
